...
#### 1️⃣ **쓰레기 제보 프로세스**
```
사용자 위치 확인 → 사진 촬영 → 설명 작성 → 마커 등록 → 지도 표시
```

#### 2️⃣ **AI 검증 프로세스**
```
AFTER 사진 업로드 → OpenAI GPT-4o 분석 → 청소 완료 판단 → 포인트 적립
```

#### 3️⃣ **포인트 시스템 프로세스**
```
청소 인증 → 기본 포인트(5,000pt) → 보너스 포인트(1,000pt) → 상품권 교환
```
...
| **포인트 교환** | `/point-exchange` | 포인트를 온누리상품권으로 교환 |
| **내 핀 관리** | `/my-pins` | 구매한 상품권 핀번호 관리 |
...
| **에러 핸들링** | 구체적 에러 메시지 | 사용자 친화적 오류 안내 |
...
| **LazyInitializationException** | ✅ 해결 | @Transactional 어노테이션 적용 |
| **컴파일 에러** | ✅ 해결 | Lombok @Getter/@Setter 명시적 추가 |
...
| **비밀번호 보안** | BCrypt 해싱 알고리즘 |
| **API 보안** | 토큰 기반 인증 미들웨어 |
| **파일 업로드** | 이미지 파일 타입 검증 |
...
| 시스템 | 구현 내용 |
|--------|-----------|
| **AI 검증 로깅** | GPT 응답, 신뢰도, 검증 결과 상세 기록 |
| **사용자 활동** | 제보, 인증, 포인트 적립 내역 추적 |
| **에러 모니터링** | 구조화된 에러 로깅 및 분석 |
| **성능 모니터링** | API 응답 시간, 처리량 측정 |
